...
contract SudokuSolvePlonkVerifier {
    ...
    uint16 constant nPublic   = 83;
    uint16 constant nLagrange = 83;
    
    uint256 constant Qmx  = 11836845416819497059941163607550035122031374122202800398647165427958616397551;
    uint256 constant Qmy  = 2807012592455718207442315670666653066780040103046959987806054456565817238643;
    uint256 constant Qlx  = 20060873995006777052263282854471296146816054259672437498287706551509399480421;
    uint256 constant Qly  = 17205704142684774266876865687805190705656990933048200899027495985886405276422;
    uint256 constant Qrx  = 7151096675109476750059120140692897383672700455383354811689923105162097914926;
    uint256 constant Qry  = 18733663079660374317486216051840152363605700977984024966950859444689848401796;
    uint256 constant Qox  = 1969947794320479802534435516729934476315051524871602088044035175968449847296;
    uint256 constant Qoy  = 2559999069304160308607114042973931413909350805229828418590045138740372393891;
    uint256 constant Qcx  = 9040402742825592098911788689409497234959863373583435679715844146241965620680;
    uint256 constant Qcy  = 2852436325923373734263881453553541650613455877421523298533596870125777518517;
    uint256 constant S1x  = 11987875277310244347545933949668064440450060317925596486847411138387939044950;
    uint256 constant S1y  = 12904348760537141836342870390690917593721115075068527772417618190401919181131;
    uint256 constant S2x  = 16363124489789771606742085335721955455953927661682633473049742197671840202850;
    uint256 constant S2y  = 13058973745031146632612224140508448646359198468630214596974541060242103700622;
    uint256 constant S3x  = 16321278842467543755011450362799213858356395397585066652561117944504049300624;
    uint256 constant S3y  = 21345960316926117669788451437326425674443382282630289542680988147277931240755;
    ...
    uint16 constant lastMem = 3456;

    function verifyProof(uint256[24] calldata _proof, uint256[83] calldata _pubSignals) public view returns (bool) {
        assembly {
            /////////
            ...
            function calculateChallenges(pMem, pPublic) {
                ...
                mstore(add(mIn, 3168 ), calldataload(pA))
                mstore(add(mIn, 3200 ), calldataload(add(pA, 32)))
                mstore(add(mIn, 3232 ), calldataload(pB))
                mstore(add(mIn, 3264 ), calldataload(add(pB, 32)))
                mstore(add(mIn, 3296 ), calldataload(pC))
                mstore(add(mIn, 3328 ), calldataload(add(pC, 32)))
                
                beta := mod(keccak256(mIn, 3360), q) 
            ...
            function calculateLagrange(pMem) {
                ...
                inverseArray(add(pMem, pZhInv), 84 )
