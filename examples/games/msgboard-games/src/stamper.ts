 ...
 *
 *   1. NATIVE  — the `@gibs/pow-grinder` Rust addon (`.node`). Fastest, but the `.node` is a build
 ...
 *   2. WASM    — the committed `@gibs/pow-grinder/wasm` module. Always present (`pow_grinder_bg.wasm`
...
async function tryNative(): Promise<EngineStamp | null> {
  try {
    ...
    const mod: { stamp?: EngineStamp } = await import('@gibs/pow-grinder')
...
async function tryWasm(): Promise<EngineStamp | null> {
  try {
    const wasm: {
      ...
      stamp: EngineStamp
    } = await import('@gibs/pow-grinder/wasm')
    if (isNode()) {
      ...
      // sibling of the resolved `@gibs/pow-grinder/wasm` JS module (robust to install layout), falling
