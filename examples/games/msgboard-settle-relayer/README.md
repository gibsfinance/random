...
- **Games settlement worker → this package.** A *composition* that wires the games
  `Settlement` builders (`@gibs/msgboard-settle`), the retained `Transcript`/`SessionState`
  types (`@gibs/msgboard-games`), and the published engine (`@msgboard/relayer`). It cannot
