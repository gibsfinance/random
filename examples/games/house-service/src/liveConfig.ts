...
/** Read the operator mnemonic from $MNEMONIC, falling back to packages/contracts/.env. */
export function readMnemonic(): string {
  const fromEnv = process.env.MNEMONIC?.trim()
  if (fromEnv) return fromEnv
  // Fallback: parse the contracts package .env (relative to this module → repo .../packages/contracts/.env).
  const envPath = fileURLToPath(new URL('../../../../packages/contracts/.env', import.meta.url))
  const m = readFileSync(envPath, 'utf8').match(/^MNEMONIC=(.+)$/m)
  const mnemonic = m?.[1]?.trim().replace(/^["']|["']$/g, '')
  if (!mnemonic) throw new Error('no MNEMONIC in env or packages/contracts/.env')
  return mnemonic
}
