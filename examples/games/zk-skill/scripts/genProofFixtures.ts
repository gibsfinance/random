...
async function genSudokuFixture(setup: CircuitSetup) {
  ...
  const witnessInput = await buildSudokuWitnessInput({ puzzle: PUZZLE, solution: SOLUTION, salt })
  const { proof, publicSignals } = await prove(setup, witnessInput)
  const ok = await verify(setup, publicSignals, proof)
  if (!ok) throw new Error('genSudokuFixture: proof failed in-process verify — aborting')
  if (publicSignals.length !== 82) {
    throw new Error(`genSudokuFixture: expected 82 public signals, got ${publicSignals.length}`)
  }
  ...
  writeFixture('sudokuSolveProof.json', {
    _comment:
      ...
      'publicSignals / the packed pub[] order is [puzzle[0..80], commit] (82 signals), matching ' +
      'circuits/sudoku_solve.circom `component main {public [puzzle, commit]}`. pA/pB/pC are already ' +
