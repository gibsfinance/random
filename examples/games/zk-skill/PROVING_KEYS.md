...
```ts
import manifest from '@gibs/zk-skill/proving-keys.manifest.json'
import { proveInWorker } from '@gibs/zk-skill/browser'
...
const { proof, publicSignals } = await proveInWorker(
  ...
  () => new Worker(new URL('@gibs/zk-skill/browser/prover.worker.js', import.meta.url), { type: 'module' }),
)
```
...
# or both at once:
pnpm --filter @gibs/zk-skill typecheck
```
