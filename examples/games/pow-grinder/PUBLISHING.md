...
| --------------------------- | --------------- | ------------ | ------------------------------------ |
| `@gibs/pow-grinder`         | native `.node`  | ~0.9–1.2s    | per-platform (built locally, opt-in) |
| `@gibs/pow-grinder/wasm`    | WASM (`pkg/`)   | ~1.2–1.8s    | **everywhere** (Node, browser, Bun)  |
...
//     'file://' + createRequire(import.meta.url).resolve('@gibs/pow-grinder/wasm'))
...
```ts
import { stamp } from '@gibs/pow-grinder'
```
